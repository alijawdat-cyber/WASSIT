...
export default function ClientDisputeDetails() {
  ...
  // التحقق من نوع المستخدم والتوجيه إذا لم يكن عميلًا
  useEffect(() => {
      ...
      const fetchDispute = async () => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            // التحقق من أن النزاع ينتمي للعميل الحالي
  ...
  const handleCancelDispute = async () => {
    ...
    if (window.confirm('هل أنت متأكد من رغبتك في إلغاء هذا النزاع؟')) {
      try {
        const response = await disputesAPI.cancelDispute(dispute.id);
        
        if (response.success) {
          // تحديث حالة النزاع في الواجهة
          setDispute({
            ...dispute,
            status: DisputeStatus.CANCELED,
          });
        } else {
          alert(response.message || 'حدث خطأ أثناء إلغاء النزاع');
        }
      } catch (err) {
        console.error('خطأ في إلغاء النزاع:', err);
        alert('حدث خطأ أثناء إلغاء النزاع');
      }
    }
  };
      ...
      <div className="container mx-auto p-4">
        <div className="mb-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <Button 
              variant="outline" 
              onClick={() => router.push('/client/disputes')}
            >
              العودة للنزاعات
            </Button>
