...
export default function ProviderRequestsPage() {
  ...
  useEffect(() => {
    ...
    // استدعاء الطلبات المتاحة لمزود الخدمة
    const fetchRequests = async () => {
      try {
        setIsLoading(true);
        setError('');
        
        // هنا نستخدم فلتر لجلب الطلبات المفتوحة فقط حيث يمكن لمزود الخدمة تقديم عروض عليها
        const response = await requestsAPI.getAllRequests({ status: RequestStatus.OPEN });
        ...
        console.error('خطأ في استرجاع الطلبات المتاحة:', err);
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">
          استعرض الطلبات المفتوحة وقدم عروضك للعملاء
        </p>
      </div>
      ...
      {requests.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requests.map((request) => (
            <Link href={`/provider/requests/${request.id}`} key={request.id}>
